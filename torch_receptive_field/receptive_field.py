...
def receptive_field(model, input_size, batch_size=-1, device="cuda"):
    """
    :parameter
    'input_size': tuple of (Channel, Seq Len)
    ...
    def register_hook(module):
        def hook(module, input, output):
            ...
            if not receptive_field["0"]["conv_stage"]:
                ...
                if class_name == "Conv1d" or class_name == "MaxPool1d":
                    ...
                    receptive_field[m_key]["r"] = (
                        p_r + ((kernel_size - 1) * dilation) * p_j
                    )
    ...
    for layer in receptive_field:
        ...
        assert len(receptive_field[layer]["output_shape"]) == 3
        line_new = "{:7} {:12}  {:>10} {:>10} {:>10} {:>15} ".format(
            "",
            layer,
            str(receptive_field[layer]["output_shape"][1:]),
...
def receptive_field_for_unit(receptive_field_dict, layer, unit_position):
    ...
    if layer in receptive_field_dict:
        ...
        feat_map_lim = rf_stats["output_shape"][1:]
        if np.any(
            [
        ...
        rf_range = [
            (max(0, rf_range[axis][0]), min(limit[axis], rf_range[axis][1]))
            for axis in range(2)
        ]
        print(
            "Receptive field size for layer %s, unit_position %s,  is \n %s"
